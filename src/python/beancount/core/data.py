...
## FIXME: Consider moving this to its own file.
...
def hash_entries(entries):
    ...
    for entry in entries:
        ...
        if entry_type is Transaction:
            ...
            hashable_postings = sorted(posting._replace(entry=None)
                                       for posting in entry.postings)
            hashable_entry = entry._replace(fileloc=None,
                                            postings=None)
            entry_pickle = pickle.dumps(hashable_entry)
            md5.update(entry_pickle)
            for hashable_posting in hashable_postings:
                posting_pickle = pickle.dumps(hashable_posting)
                print(posting_pickle)
                md5.update(posting_pickle)
        else:
            hashable_entry = entry._replace(fileloc=None)
            entry_pickle = pickle.dumps(hashable_entry)
            md5.update(entry_pickle)
...
def compare_entries(entries1, entries2):
    ...
    Returns:
      A tuple of (success, not_found1, not_found2), where the fields are:
        ...
        not_found1: A list of directives from 'entries1' not found in
          'entries2'.
        not_found2: A list of directives from 'entries2' not found in
          'entries1'.
    """
    hash1 = hashlib.md5()
    hash2 = hashlib.md5()
...
def includes_entries(entries, subset_entries):
    pas
