...
def context_from_numbers(numbers):
    dcontext = display_context.DisplayContext()
    for number in numbers:
        dcontext.update(Decimal(number))
    return dcontext
...
class TestDisplayContext(unittest.TestCase):
    ...
    def test_no_clear_mode(self):
        numbers = [Decimal(x) for x in ['1.2345', '764', '-7409.01', '0.00000125']]
        dcontext = context_from_numbers(numbers)
        numfmt = dcontext.build()
...
0.00000125
        ...
        print(numfmt.fmtstrings)
        for number in numbers:
            print(numfmt.format(number))
