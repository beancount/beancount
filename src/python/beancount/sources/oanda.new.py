...
def guess_currency(filename):
    ...
    for obj in csv_utils.csv_dict_reader(open(filename), delimiter='\t'):
...
def import_csv_file(filename, config):
    ...
    for lineno, obj in enumerate(csv_utils.csv_dict_reader(open(filename), delimiter='\t')):
        ...
        if is_balanced_txn(obj):
            # Late 2013, OANDA decided to break this invariant, why couldn't
            # they release a new version keeping the old one at the same time?
