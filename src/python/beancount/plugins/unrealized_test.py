...
class TestUnrealized(unittest.TestCase):
    ...
    def test_normal_case(self, entries, _, options_map):
        ...
        self.assertEqual(D('350'), house.postings[0].position.units.number)
        ...
        self.assertEqual(D('-100'), mansion.postings[0].position.units.number)
    ...
    def test_no_price(self, entries, _, options_map):
        ...
        self.assertEqual(ZERO, unreal_entries[0].postings[0].position.units.number)
    ...
    def test_immediate_profit(self, entries, _, options_map):
        ...
        self.assertEqual(D('200'),
                         unreal_entries[0].postings[0].position.units.number)
    ...
    def test_not_assets(self, entries, _, options_map):
        ...
        self.assertEqual("Assets:Account1:Gains", entry.postings[0].account)
        self.assertEqual("Income:Account1:Gains", entry.postings[1].account)
        self.assertEqual(D("10.00"), entry.postings[0].position.units.number)
        self.assertEqual(D("-10.00"), entry.postings[1].position.units.number)
        ...
        self.assertEqual("Income:Account1:Gains", entry.postings[1].account)
        self.assertEqual(D("18.00"), entry.postings[0].position.units.number)
        self.assertEqual(D("-18.00"), entry.postings[1].position.units.number)

        entry = get_entries_with_narration(unreal_entries, '3 units')[0]
        self.assertEqual("Equity:Account1:Gains", entry.postings[0].account)
        self.assertEqual("Income:Account1:Gains", entry.postings[1].account)
        self.assertEqual(D("24.00"), entry.postings[0].position.units.number)
        self.assertEqual(D("-24.00"), entry.postings[1].position.units.number)
        ...
        self.assertEqual("Income:Account1:Gains", entry.postings[1].account)
        self.assertEqual(D("28.00"), entry.postings[0].position.units.number)
        self.assertEqual(D("-28.00"), entry.postings[1].position.units.number)

        entry = get_entries_with_narration(unreal_entries, '5 units')[0]
        self.assertEqual("Income:Account1:Gains", entry.postings[0].account)
        self.assertEqual("Income:Account1:Gains", entry.postings[1].account)
        self.assertEqual(D("30.00"), entry.postings[0].position.units.number)
        self.assertEqual(D("-30.00"), entry.postings[1].position.units.number)
