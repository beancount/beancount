...
class TestCase(unittest.TestCase):
    ...
    def assertEqualEntries(self, expected_entries, actual_entries):
        ...
        same, expected_missing, actual_missing = copmare.compare_entries(expected_entries,
                                                                         actual_entries)
        if not same:
    ...
    def assertIncludesEntries(self, expected_entries, actual_entries):
        ...
        if isinstance(actual_entries, str):
            actual_entries, _, __ = parser.parse_string(actual_entries)
        includes, missing = copmare.includes_entries(expected_entries, actual_entries)
        if not includes:
            assert missing
            oss = io.StringIO()
            if missing:
