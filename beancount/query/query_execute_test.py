...
class QueryBase(cmptest.TestCase):
    ...
    def compile(self, bql_string):
        ...
        """
        return qc.compile_select(
            self.parse(bql_string),
    ...
    def check_sorted_query(self, input_string, bql_string, expected_types, expected_rows):
        return self.check_query(
            input_string, bql_string, expected_types, expected_rows, True
        )
...
class TestFilterEntries(CommonInputBase, QueryBase):
    def test_filter_empty_from(self):
        ...
        filtered_entries = qx.filter_entries(
            self.compile("""
          SELECT * ;
        """).c_from,
            self.entries,
...
class TestExecutePrint(CommonInputBase, QueryBase):
    def test_print_with_filter(self):
        statement = qc.EvalPrint(
            qc.EvalFrom(
                qc.EvalEqual(qe.YearEntryColumn(), qc.EvalConstant(2012)), None, None, None
            )
        )
        oss = io.StringIO()
...
class TestExecuteNonAggregatedQuery(QueryBase):
    ...
    def test_non_aggregate__one(self):
        self.check_query(
            ...
            """,
            [("date", datetime.date)],
            [(datetime.date(2010, 2, 23),), (datetime.date(2010, 2, 23),)],
        )
    ...
    def test_aggregated_group_by_invisible_order_by_non_aggregate_visible(self):
        ...
        self.check_query(
            ...
            [
                (
                    19,
                    inventory.from_string("-100.00 USD"),
                ),
                (
                    20,
                    inventory.from_string("100.00 USD"),
