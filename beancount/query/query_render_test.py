...
class TestAmountRenderer(ColumnRendererBase):
    ...
    def test_single_int(self):
        pos = A("5 HOOL")
        rdr = self.get(pos)
        self.assertEqual("5     HOOL", rdr.format(pos))

    def test_many(self):
        amounts = [A(x) for x in ("0.0001 USD", "20.002 HOOL", "33 CA", "1098.20 AAPL")]
        rdr = self.get(*amounts)
        self.assertEqual(
            ["   0.0001 USD ", "  20.002  HOOL", "  33      CA  ", "1098.20   AAPL"],
...
class TestInventoryRenderer(ColumnRendererBase):
    ...
    def test_various(self):
        ...
        rdr = self.get(inv)
        self.assertEqual(
            [" 5      HOOL {500.23   USD}", "12.3456 CAAD               "], rdr.format(inv)
        )
