...
class PayeeEntryColumn(query_compile.EvalColumn):
    ...
    def __call__(self, context):
        return context.entry.payee or "" if isinstance(context.entry, Transaction) else None
...
class DescriptionEntryColumn(query_compile.EvalColumn):
    ...
    def __call__(self, context):
        return (
            " | ".join(filter(None, [context.entry.payee, context.entry.narration]))
            if isinstance(context.entry, Transaction)
            else None
        )
...
class TagsEntryColumn(query_compile.EvalColumn):
    ...
    def __call__(self, context):
        return (
            context.entry.tags or EMPTY_SET
            if isinstance(context.entry, Transaction)
            else EMPTY_SET
        )
...
class LinksEntryColumn(query_compile.EvalColumn):
    ...
    def __call__(self, context):
        return (
            context.entry.links or EMPTY_SET
            if isinstance(context.entry, Transaction)
            else EMPTY_SET
        )
...
class FileLocationColumn(query_compile.EvalColumn):
    ...
    def __call__(self, context):
        if context.posting.meta is not None:
            return "{}:{:d}:".format(
                context.posting.meta.get("filename", "N/A"),
...
class DescriptionColumn(query_compile.EvalColumn):
    ...
    def __call__(self, context):
        entry = context.entry
        return (
            " | ".join(filter(None, [entry.payee, entry.narration]))
