...
class TestSelectTarget(QueryParserTestBase):
    ...
    def test_target_one(self):
        self.assertParse(qSelect([qp.Target(qp.Column("date"), None)]), "SELECT date;")

    def test_target_one_as(self):
        self.assertParse(
            qSelect([qp.Target(qp.Column("date"), "xdate")]), "SELECT date as xdate;"
        )
...
class TestSelectExpression(QueryParserTestBase):
    def test_expr_constant_null(self):
        self.assertParse(qSelect([qp.Target(qp.Constant(None), None)]), "SELECT NULL;")

    def test_expr_constant_boolean(self):
        self.assertParse(qSelect([qp.Target(qp.Constant(True), None)]), "SELECT TRUE;")

        self.assertParse(qSelect([qp.Target(qp.Constant(False), None)]), "SELECT FALSE;")

    def test_expr_constant_integer(self):
        self.assertParse(qSelect([qp.Target(qp.Constant(17), None)]), "SELECT 17;")

    def test_expr_constant_decimal(self):
        self.assertParse(
            qSelect([qp.Target(qp.Constant(D("17.345")), None)]), "SELECT 17.345;"
        )

        self.assertParse(qSelect([qp.Target(qp.Constant(D(".345")), None)]), "SELECT .345;")

        self.assertParse(qSelect([qp.Target(qp.Constant(D("17.")), None)]), "SELECT 17.;")

    def test_expr_constant_string(self):
        self.assertParse(
            qSelect([qp.Target(qp.Constant("rainy-day"), None)]), "SELECT 'rainy-day';"
        )
    ...
    def test_expr_ne(self):
        self.assertParse(
            qSelect([qp.Target(qp.Not(qp.Equal(qp.Column("a"), qp.Constant(42))), None)]),
            "SELECT a != 42;",
        )
    ...
    def test_expr_match(self):
        self.assertParse(
            qSelect(
                [
                    qp.Target(
                        qp.Match(qp.Column("a"), qp.Constant("Assets:.*:Checking")), None
                    )
                ]
            ),
    ...
    def test_expr_not(self):
        self.assertParse(
            qSelect([qp.Target(qp.Not(qp.Column("a")), None)]), "SELECT NOT a;"
        )
    ...
    def test_expr_function__zero_args(self):
        self.assertParse(
            qSelect([qp.Target(qp.Function("random", []), None)]), "SELECT random();"
        )
    ...
    def test_expr_function__two_args(self):
        self.assertParse(
            qSelect(
                [qp.Target(qp.Function("min", [qp.Column("a"), qp.Column("b")]), None)]
            ),
            "SELECT min(a, b);",
        )
...
class TestSelectPrecedence(QueryParserTestBase):
    ...
    def test_expr_function__and_not(self):
        self.assertParse(
            qSelect(
                qp.Wildcard(), where_clause=qp.And(qp.Not(qp.Column("a")), qp.Column("b"))
            ),
...
class TestSelectFromBase(QueryParserTestBase):
    def setUp(self):
        super().setUp()
        self.targets = [qp.Target(qp.Column("a"), None), qp.Target(qp.Column("b"), None)]
        self.expr = qp.Equal(
            qp.Column("d"), qp.And(qp.Function("max", [qp.Column("e")]), qp.Constant(17))
        )
...
class TestSelectFrom(TestSelectFromBase):
    ...
    def test_from_open_default(self):
        self.assertParse(
            qSelect(
                self.targets, qp.From(self.expr, datetime.date(2014, 1, 1), None, None)
            ),
    ...
    def test_from_close_default(self):
        self.assertParse(
            qSelect(self.targets, qp.From(self.expr, None, True, None)),
    ...
    def test_from_close_dated(self):
        self.assertParse(
            qSelect(
                self.targets, qp.From(self.expr, None, datetime.date(2014, 10, 18), None)
            ),
    ...
    def test_from_close_no_expr_dated(self):
        self.assertParse(
            qSelect(self.targets, qp.From(None, None, datetime.date(2014, 10, 18), None)),
...
class TestSelectWhere(TestSelectFromBase):
    ...
    def test_where(self):
        self.assertParse(
            qSelect(self.targets, None, self.expr), "SELECT a, b WHERE d = (max(e) and 17);"
        )
...
class TestSelectFromAndWhere(TestSelectFromBase):
    def test_both(self):
        self.assertParse(
            qSelect(self.targets, qp.From(self.expr, None, None, None), self.expr),
            """
          SELECT a, b
          FROM d = (max(e) and 17)
...
class TestSelectFromSelect(QueryParserTestBase):
    def test_from_select(self):
        subselect = qSelect(
            qp.Wildcard(),
            qp.From(
                qp.Equal(qp.Column("date"), qp.Constant(datetime.date(2014, 5, 2))),
                None,
                None,
                None,
            ),
        ...
        expected = qSelect(
            [qp.Target(qp.Column("a"), None), qp.Target(qp.Column("b"), None)],
            subselect,
...
class TestSelectGroupBy(QueryParserTestBase):
    ...
    def test_groupby_one(self):
        self.assertParse(
            qSelect(qp.Wildcard(), group_by=qp.GroupBy([qp.Column("a")], None)),
            "SELECT * GROUP BY a;",
        )
    ...
    def test_groupby_many(self):
        self.assertParse(
            qSelect(
                qp.Wildcard(),
                group_by=qp.GroupBy([qp.Column("a"), qp.Column("b"), qp.Column("c")], None),
            ),
    ...
    def test_groupby_having(self):
        self.assertParse(
            qSelect(
                ...
                group_by=qp.GroupBy(
                    [qp.Column("a")],
                    qp.Equal(qp.Function("sum", [qp.Column("position")]), qp.Constant(0)),
                ),
            ),
    ...
    def test_groupby_numbers(self):
        self.assertParse(
            qSelect(qp.Wildcard(), group_by=qp.GroupBy([1], None)), "SELECT * GROUP BY 1;"
        )
...
class TestSelectOrderBy(QueryParserTestBase):
    ...
    def test_orderby_one(self):
        self.assertParse(
            qSelect(qp.Wildcard(), order_by=qp.OrderBy([qp.Column("a")], None)),
            "SELECT * ORDER BY a;",
        )
    ...
    def test_orderby_many(self):
        self.assertParse(
            qSelect(
                qp.Wildcard(),
                order_by=qp.OrderBy([qp.Column("a"), qp.Column("b"), qp.Column("c")], None),
            ),
    ...
    def test_orderby_asc(self):
        self.assertParse(
            qSelect(qp.Wildcard(), order_by=qp.OrderBy([qp.Column("a")], "ASC")),
            "SELECT * ORDER BY a ASC;",
        )

    def test_orderby_desc(self):
        self.assertParse(
            qSelect(qp.Wildcard(), order_by=qp.OrderBy([qp.Column("a")], "DESC")),
...
class TestSelectPivotBy(QueryParserTestBase):
    ...
    def test_pivotby_many(self):
        self.assertParse(
            qSelect(
                qp.Wildcard(),
                pivot_by=qp.PivotBy([qp.Column("a"), qp.Column("b"), qp.Column("c")]),
            ),
...
class TestSelectOptions(QueryParserTestBase):
    def test_distinct(self):
        self.assertParse(
            qSelect([qp.Target(qp.Column("account"), None)], distinct=True),
    ...
    def test_limit_present(self):
        self.assertParse(qSelect(qp.Wildcard(), limit=45), "SELECT * LIMIT 45;")

    def test_flatten(self):
        self.assertParse(qSelect(qp.Wildcard(), flatten=True), "SELECT * FLATTEN;")

    def test_limit_and_flatten(self):
        self.assertParse(
            qSelect(qp.Wildcard(), limit=100, flatten=True), "SELECT * LIMIT 100 FLATTEN;"
        )
...
class TestJournal(QueryParserTestBase):
    ...
    def test_journal_account(self):
        self.assertParse(
            qp.Journal("Assets:Checking", None, None), "JOURNAL 'Assets:Checking';"
        )
    ...
    def test_journal_account_and_summary(self):
        self.assertParse(
            qp.Journal("Assets:Checking", "cost", None),
...
class TestPrint(QueryParserTestBase):
    def test_print_empty(self):
        self.assertParse(qp.Print(None), "PRINT;")
...
class TestExpressionName(QueryParserTestBase):
    ...
    def test_constant(self):
        ...
        self.assertEqual(
            "c2014_01_01", qp.get_expression_name(qp.Constant(datetime.date(2014, 1, 1)))
        )

    def test_unary(self):
        self.assertEqual(
            "not_account", qp.get_expression_name(qp.Not(qp.Column("account")))
        )

    def test_binary(self):
        self.assertEqual(
            "and_account_date",
            qp.get_expression_name(qp.And(qp.Column("account"), qp.Column("date"))),
        )
...
class TestExplain(QueryParserTestBase):
    ...
    def test_explain_balances(self):
        self.assertParse(
            qp.Explain(qp.Balances("cost", None, None)), "EXPLAIN BALANCES AT cost;"
        )
