...
class EvalNode:
    ...
    def __str__(self):
        return "{}({})".format(
            type(self).__name__,
            ", ".join(repr(getattr(self, child)) for child in self.__slots__),
        )
...
class CompilationEnvironment:
    ...
    def get_column(self, name):
        ...
        try:
            ...
            raise CompilationError(
                "Invalid column name '{}' in {} context.".format(name, self.context_name)
            ) from exc
    ...
    def get_function(self, name, operands):
        ...
        try:
            ...
            try:
                ...
                signature = "{}({})".format(
                    name, ", ".join(operand.dtype.__name__ for operand in operands)
                )
                raise CompilationError(
                    "Invalid function '{}' in {} context".format(
...
def compile_expression(expr, environ):
    ...
    if isinstance(expr, query_parser.Column):
        ...
        c_operands = [compile_expression(operand, environ) for operand in expr.operands]
        ...
        node_type = OPERATORS[type(expr)]
        c_expr = node_type(
            compile_expression(expr.left, environ), compile_expression(expr.right, environ)
        )
...
def compile_targets(targets, environ):
    ...
    for target in targets:
        ...
        target_name = find_unique_name(
            target.name or query_parser.get_expression_name(target.expression), target_names
        )
    ...
    if group_by:
        ...
        for column in group_by.columns:
            ...
            if is_aggregate(c_expr):
                raise CompilationError(
                    "GROUP-BY expressions may not reference aggregates: '{}'".format(column)
                )

            # Check that the group-by column has a supported hashable type.
            if not is_hashable_type(c_expr):
                raise CompilationError(
                    "GROUP-BY a non-hashable type is not supported: '{}'".format(column)
                )
...
def compile_order_by(order_by, c_targets, environ):
    ...
    for column in order_by.columns:
        ...
        if isinstance(column, int):
            index = column - 1
            if not (0 <= index < len(c_targets)):
                raise CompilationError("Invalid ORDER-BY column index {}".format(column))
...
def compile_from(from_clause, environ):
    ...
    if from_clause is not None:
        c_expression = (
            compile_expression(from_clause.expression, environ)
        ...
        if (
            isinstance(from_clause.open, datetime.date)
            and isinstance(from_clause.close, datetime.date)
            and from_clause.open > from_clause.close
        ):
            raise CompilationError("Invalid dates: CLOSE date must follow OPEN date")

        c_from = EvalFrom(
            c_expression, from_clause.open, from_clause.close, from_clause.clear
        )
    else:
        c_from = None
...
def compile_select(select, targets_environ, postings_environ, entries_environ):
    ...
    new_targets, group_indexes = compile_group_by(
        select.group_by, c_targets, environ_target
    )
    if new_targets:
    ...
    if select.order_by is not None:
        (new_targets, order_indexes) = compile_order_by(
            select.order_by, c_targets, environ_target
        )
        if new_targets:
            c_targets.extend(new_targets)
    ...
    if group_indexes is not None:
        non_aggregate_indexes = set(
            index for index, c_target in enumerate(c_targets) if not c_target.is_aggregate
        )
        if non_aggregate_indexes != set(group_indexes):
            missing_names = [
                '"{}"'.format(c_targets[index].name)
...
def transform_journal(journal):
    ...
    """.format(
            where=(
                'WHERE account ~ "{}"'.format(journal.account) if journal.account else ""
            ),
            summary_func=journal.summary_func or "",
        )
    )

    return query_parser.Select(
        cooked_select.targets,
        journal.from_clause,
