...
COMMANDS = [
    ...
    ("journal", ("bean-report {} journal -w 120 " "-a Assets:US:BofA:Checking")),
    (
        "journal-with-balance",
        ("bean-report {} journal -w 120 " "-a Assets:US:BofA:Checking -b"),
    ),
    ("invest", ("bean-report {} journal -w 120 " "-a Assets:US:ETrade:GLD -b")),
    (
        "invest-with-cost",
        ("bean-report {} journal -w 120 " "-a Assets:US:ETrade:GLD -b -c"),
    ),
...
def main():
    ...
    for report_name, command_template in COMMANDS:
        ...
        if stderr.getvalue():
            errors_filename = path.join(
                args.output_directory, "{}.errors".format(report_name)
            )
