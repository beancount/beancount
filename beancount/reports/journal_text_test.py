...
class TestJournalRenderPosting(unittest.TestCase):
    ...
    def test_render_posting_no_cost(self):
        ...
        str_posting = journal_text.render_posting(
            data.Posting("Assets:Something", pos.units, pos.cost, None, None, None),
            self.number_format,
        )
        self.assertEqual("  Assets:Something                 100 USD", str_posting)
    ...
    def test_render_posting_price(self):
        ...
        str_posting = journal_text.render_posting(
            data.Posting(
                "Assets:Something", pos.units, pos.cost, A("45.32 USD"), None, None
            ),
            self.number_format,
        )
        self.assertEqual(
    ...
    def test_render_posting_cost_price(self):
        ...
        str_posting = journal_text.render_posting(
            data.Posting(
                "Assets:Something", pos.units, pos.cost, A("47.00 USD"), None, None
            ),
            self.number_format,
        )
        self.assertEqual(
            "  Assets:Something                 10 VHT {45.32 USD} @ 47.00 USD", str_posting
        )
...
class TestJournalTextRender(unittest.TestCase):
    ...
    def test_get_entry_text_description(self):
        ...
        for expected_description, entry in zip(expected_descriptions, self.entries):
            self.assertEqual(
                expected_description, journal_text.get_entry_text_description(entry)
            )
