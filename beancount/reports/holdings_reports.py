...
def get_holdings_entries(entries, options_map):
    ...
    for index, holding in enumerate(holdings_list):
        ...
        entry = data.Transaction(
            meta, latest_date, flags.FLAG_SUMMARIZE, None, "", None, None, []
        )
        ...
        entry.postings.append(data.Posting(equity_account, cost, None, None, None, None))
    ...
    used_open_entries = [
        open_entry for open_entry in open_entries if open_entry.account in used_accounts
    ]

    # Add an entry for the equity account we're using.
    meta = data.new_metadata("report_holdings_print", -1)
    used_open_entries.insert(0, data.Open(meta, latest_date, equity_account, None, None))
...
def load_from_csv(fileobj):
    ...
    column_dict = {name: (attr, converter) for name, attr, converter in column_spec}
...
class HoldingsReport(base.TableReport):
    ...
    def generate_table(self, entries, errors, options_map):
        ...
        return report_holdings(
            self.args.currency, self.args.relative, entries, options_map, **keywords
        )
...
class NetWorthReport(base.TableReport):
    ...
    def generate_table(self, entries, errors, options_map):
        ...
        return table.create_table(calculate_net_worths(entries, options_map), field_spec)
