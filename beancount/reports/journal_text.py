...
class AmountColumnSizer:
    ...
    def get_number_width(self):
        ...
        """
        return (math.floor(math.log10(self.max_number)) + 1) if self.max_number > 0 else 1
...
def text_entries_table(
    ...
    entry_data, change_sizer, balance_sizer = size_and_render_amounts(
    ...
    empty_line = empty_format.format(
        date="", dirtype="", description="", change="", balance=""
    )
    ...
    line_format = "{{date:10}} {{dirtype:5}} {{description:{:d}.{:d}}}  {}".format(
        description_width, description_width, change_sizer.get_generic_format(precision)
    )
    ...
    for entry, leg_postings, change_amounts, balance_amounts in entry_data:
        ...
        for description, change_amount, balance_amount in itertools.zip_longest(
            description_lines, change_amounts, balance_amounts, fillvalue=""
        ):
            change = (
                change_format.format(change_amount.number, change_amount.currency)
                if change_amount
                else ""
            )

            balance = (
                balance_format.format(balance_amount.number, balance_amount.currency)
                if balance_amount
                else ""
            )
            ...
            if output_format is FORMAT_TEXT:
                ...
                if render_balance:
                    ...
                    row = (date, dirtype, description, change_number, change_currency)
...
def render_posting(posting, number_format):
    ...
    if cost:
        strings.append(
            "{{{}}}".format(number_format.format(cost.number, cost.currency).strip())
        )
...
def get_entry_text_description(entry):
    ...
    if isinstance(entry, data.Transaction):
        description = " | ".join(
            [field for field in [entry.payee, entry.narration] if field is not None]
        )
    elif isinstance(entry, data.Balance):
        if entry.diff_amount is None:
            description = "PASS - In {}".format(entry.account)
        else:
            description = ("FAIL - In {}; " "expected = {}, difference = {}").format(
                entry.account, entry.amount, entry.diff_amount
            )
