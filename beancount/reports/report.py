...
def get_all_reports():
    ...
    return functools.reduce(
        operator.add,
        map(
            lambda module: module.__reports__,
            [
                balance_reports,
                journal_reports,
                holdings_reports,
...
class ListFormatsAction(argparse.Action):
    ...
    format_order = {"text": 1, "html": 2, "htmldiv": 3, "csv": 4, "beancount": 99}
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        all_formats = sorted(
            {format_ for name, formats in matrix for format_ in formats},
...
def main(argv=None):
    ...
    parser.add_argument(
        "-t", "--timings", "--verbose", action="store_true", help="Print timings."
    )
    ...
    for report_class in get_all_reports():
        ...
        report_parser = subparsers.add_parser(
            name, aliases=aliases, description=oss.getvalue()
        )
    ...
    if hasattr(args, "filters") and args.filters:
        parser.error(
            (
                ...
                "See bean-query if you need filtering now."
            ).format(args.filters)
        )
