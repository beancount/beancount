    ...
    def render_htmldiv(self, entries, errors, options_map, file):
        ...
        for error in errors:
            ...
            if error.entry is not None:
                ...
                file.write(
                    textwrap.indent(printer.format_entry(error.entry, dcontext), "  ")
                )
                file.write("</pre>\n")
            file.write("</div>\n")
        file.write("</div>\n")
...
class AccountsReport(base.Report):
    ...
    def render_beancount(self, entries, errors, options_map, file):
        ...
        sortkey_fun = functools.partial(
            account_types.get_account_sort_key, options.get_account_types(options_map)
        )
        for account, (open, close) in sorted(
            open_close.items(), key=lambda entry: sortkey_fun(entry[0])
        ):
...
class CurrentEventsReport(base.TableReport):
    ...
    def generate_table(self, entries, errors, options_map):
        ...
        return table.create_table(
            list(sorted(events.items())),
            [(0, "Type", self.formatter.render_event_type), (1, "Description")],
        )
...
class EventsReport(base.TableReport):
    ...
    def generate_table(self, entries, errors, options_map):
        ...
        return table.create_table(
            [(entry.date, entry.type, entry.description) for entry in event_entries],
            [(0, "Date", datetime.date.isoformat), (1, "Type"), (2, "Description")],
        )
...
class ActivityReport(base.HTMLReport, metaclass=base.RealizationMeta):
    ...
    def render_real_htmldiv(self, real_root, price_map, price_date, options_map, file):
        ...
        for root in (account_types.assets, account_types.liabilities):
            oss = io.StringIO()
            table = tree_table.tree_table(
                oss, realization.get(real_root, root), None, ["Account", "Last Entry"]
            )
            num_cells = 0
            for real_account, cells, row_classes in table:
                ...
                cells.append(
                    data.get_entry(last_posting).date if real_account.txn_postings else "-"
                )
                num_cells += 1
...
class StatsDirectivesReport(base.TableReport):
    ...
    def generate_table(self, entries, _, __):
        entries_by_type = misc_utils.groupby(lambda entry: type(entry).__name__, entries)
        nb_entries_by_type = {
            name: len(entries) for name, entries in entries_by_type.items()
        }
        rows = sorted(nb_entries_by_type.items(), key=lambda x: x[1], reverse=True)
        rows = [(name, str(count)) for (name, count) in rows]
        rows.append(("~Total~", str(len(entries))))

        return table.create_table(rows, [(0, "Type"), (1, "Num Entries", "{:>}".format)])
...
class StatsPostingsReport(base.TableReport):
    ...
    def generate_table(self, entries, _, __):
        ...
        nb_postings_by_account = {
            key: len(postings) for key, postings in postings_by_account.items()
        }
        ...
        return table.create_table(
            rows, [(0, "Account"), (1, "Num Postings", "{:>}".format)]
        )
