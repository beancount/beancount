    ...
    def render_text(self, entries, errors, options_map, file):
        ...
        if self.args.compress_days:
            lifetimes_map = lifetimes.compress_lifetimes_days(
                lifetimes_map, self.args.compress_days
            )

        name_map = {
            pair: "{}/{}".format(pair[0], pair[1]) if pair[1] else pair[0]
            for pair in lifetimes_map.keys()
        }
        ccywidth = max(map(len, name_map.values()))
        for currency, lifetime in sorted(
            lifetimes_map.items(), key=lambda x: (x[1][0][0], x[0])
        ):
            file.write(
                "{:{width}}: {}\n".format(
                    name_map[currency],
                    "  /  ".join(
                        "{} - {}".format(begin, end or "") for begin, end in lifetime
                    ),
                    width=ccywidth,
                )
            )
...
class CommodityPricesReport(base.TableReport):
    ...
    def get_date_rates(self, entries):
        ...
        if not re.match("{ccy}/{ccy}$".format(ccy=amount.CURRENCY_RE), self.args.commodity):
            self.parser.error(
                ('Invalid commodity pair "{}"; ' "must be in BASE/QUOTE format").format(
                    self.args.commodity
                )
            )
        price_map = prices.build_price_map(entries)
    ...
    def generate_table(self, entries, errors, options_map):
        ...
        return table.create_table(
            date_rates,
            [(0, "Date", datetime.date.isoformat), (1, "Price", "{:.5f}".format)],
        )

    def render_htmldiv(self, entries, errors, options_map, file):
        date_rates = self.get_date_rates(entries)
        dates, rates = zip(*date_rates)
        scripts = gviz.gviz_timeline(
            dates, {"rates": rates, "rates2": rates}, css_id="chart"
        )
...
class PricesReport(base.Report):
    ...
    def render_beancount(self, entries, errors, options_map, file):
        price_entries = [entry for entry in entries if isinstance(entry, data.Price)]
...
class TickerReport(base.TableReport):
    ...
    def generate_table(self, entries, errors, options_map):
        ...
        return table.create_table(
            price_rows, [(0, "Currency"), (1, "Cost-Currency"), (2, "Symbol"), (3, "Name")]
        )
