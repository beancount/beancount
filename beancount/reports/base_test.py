...
class ExampleTableReport(base.TableReport):
    ...
    def generate_table(self, entries, errors, options_map):
        ...
        return table.create_table([ABC("account1", D(2000)), ABC("account2", D(5000))])
...
class TestTableReport(unittest.TestCase):
    ...
    def test_table__render_text(self):
        ...
        self.assertTrue(
            all(
                re.search(x, output) for x in ["account1", "account2", "Account", "Balance"]
            )
        )
...
class TestRealizationMeta(unittest.TestCase):
    def test_realization_metaclass(self):
        class MyReport(base.Report, metaclass=base.RealizationMeta):
            ...
            def render_real_text(
                self, real_account, price_map, price_date, options_map, file
            ):
            ...
            def render_real_html(
                self, real_account, price_map, price_date, options_map, file
            ):
                self.render_real_text(
                    real_account, price_map, price_date, options_map, file
                )
