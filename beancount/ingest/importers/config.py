...
class ConfigImporterMixin:
    ...
    def __init__(self, config):
        ...
        if not self._verify_config(config):
            raise ValueError(
                "Invalid config {}, requires {}".format(config, self.REQUIRED_CONFIG)
            )
        self.config = config
    ...
    def _verify_config(self, config):
        ...
        for option in required_options - provided_options:
            logging.error(
                "Missing value from user configuration for importer %s: %s",
                self.name(),
                option,
            )
            success = False
        ...
        for option in provided_options - required_options:
            logging.error(
                "Unknown value in user configuration for importer %s: %s",
                self.name(),
                option,
            )
            success = False
