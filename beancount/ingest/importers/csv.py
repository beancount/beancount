...
class Importer(identifier.IdentifyMixin, filing.FilingMixin):
    ...
    def file_date(self, file):
        ...
        if Col.DATE in iconfig:
            reader = iter(
                csv.reader(
                    open(file.name, encoding=self.encoding), dialect=self.csv_dialect
                )
            )
    ...
    def extract(self, file, existing_entries=None):
        ...
        reader = iter(
            csv.reader(open(file.name, encoding=self.encoding), dialect=self.csv_dialect)
        )
        ...
        for index, row in enumerate(reader, 1):
            ...
            narration = self.narration_sep.join(field.strip() for field in fields).replace(
                "\n", "; "
            )
            ...
            if txn_date is not None:
                meta["date"] = parse_date_liberally(txn_date, self.dateutil_kwds)
...
def normalize_config(config, head, dialect="excel", skip_lines: int = 0):
    ...
    if has_header:
        ...
        field_map = {field_name.strip(): index for index, field_name in enumerate(header)}
        index_config = {}
