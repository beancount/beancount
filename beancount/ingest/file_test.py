...
class TestScriptFile(scripts_utils.TestScriptsBase, test_utils.TestCase):
    ...
    def test_file_main__default_output_dir(self, file_mock):
        with test_utils.capture("stdout", "stderr") as (stdout, stderr):
            test_utils.run_with_args(
                file_main,
                ["--dry-run", path.join(self.tempdir, "test.import"), self.tempdir],
                file.__file__,
            )
        ...
        with test_utils.capture("stdout", "stderr") as (stdout, stderr):
            with self.assertRaises(SystemExit):
                test_utils.run_with_args(
                    ...
                    [
                        "--output",
                        path.join(self.documents, "Bogus"),
    ...
    def test_file__ambiguous_accounts(self, move_mock, error_mock):
        ...
        self.assertTrue(
            all(
                re.match("Ambiguous accounts", call[1][0]) for call in error_mock.mock_calls
            )
        )
    ...
    def test_file__date_uses_mtime(self, move_mock, error_mock):
        ...
        file.file(
            [imp], path.join(self.downloads, "ofxdownload.ofx"), self.documents, mkdirs=True
        )
        ...
        self.assertEqual(
            "{0:%Y-%m-%d}.ofxdownload.ofx".format(datetime.date.today()), dest_filename
        )
    ...
    def test_file__dest_dir_does_not_exist(self, move_mock, error_mock):
        ...
        self.assertTrue(
            all(
                re.match("Destination directory .* does not exist", call[1][0])
    ...
    def test_file__overwrite(self, move_mock, error_mock):
        ...
        self.assertTrue(
            all(
                re.match("Destination file .* already exist", call[1][0])
                for call in error_mock.mock_calls
            )
        )
        ...
        file.file(
            [imp],
            path.join(self.downloads, "ofxdownload.ofx"),
            self.documents,
            overwrite=True,
        )
        self.assertEqual(1, move_mock.call_count)
    ...
    def test_file__collision_in_renamed_files(self, move_mock, error_mock):
        ...
        self.assertTrue(
            all(
                re.match("Collision in destination filenames", call[1][0])
                for call in error_mock.mock_calls
            )
        )
    ...
    def test_file__file_account_raises_exception(self, error_mock):
        ...
        file.file_one_file(
            path.join(self.downloads, "ofxdownload.ofx"), [imp], self.documents
        )
    ...
    def test_file(self):
        ...
        expected_res = [
            ...
            for x in [
                r"Liabilities/CreditCard/\d\d\d\d-\d\d-\d\d\.bank\.csv",
