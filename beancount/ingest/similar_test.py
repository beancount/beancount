...
class TestDups(cmptest.TestCase):
    ...
    def test_find_similar_entries(self, entries, _, __):
        ...
        for days, num_comparisons in [(0, 1), (1, 1), (2, 1)]:
            duplicates = similar.find_similar_entries(
                new_entries, entries, lambda e1, e2: True, window_days=days
            )
            self.assertEqual(num_comparisons, len(duplicates))

            duplicates = similar.find_similar_entries(
                new_entries, entries, lambda e1, e2: False, window_days=days
            )
    ...
    def test_amounts_map(self, entries, _, __):
        ...
        self.assertEqual(
            {("Expenses:Tips", "USD"): D("1.03"), ("Expenses:Alcohol", "USD"): D("20.00")},
            amap,
        )
