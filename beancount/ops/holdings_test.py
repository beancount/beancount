...
class TestHoldings(unittest.TestCase):
    ...
    def test_get_final_holdings(self, entries, _, __):
        ...
        expected_values = [
            ...
            if (holding[0].startswith("Assets") or holding[0].startswith("Liabilities"))
        ]
        self.assertEqual(expected_values, holdings_list)
        ...
        holdings_list = sorted(map(tuple, holdings_list))
        expected_values = [
            holding for holding in expected_values if holding[0].startswith("Assets")
        ]
        self.assertEqual(expected_values, holdings_list)
    ...
    def test_get_final_holdings_with_prices(self, entries, _, __):
        ...
        holdings_list = holdings.get_final_holdings(
            entries, ("Assets", "Liabilities"), price_map
        )
    ...
    def test_get_commodities_at_date(self, entries, _, options_map):
        ...
        commodities = holdings.get_commodities_at_date(
            entries, options_map, date=datetime.date(2013, 4, 2)
        )
        self.assertEqual(
            [("HOOL", "USD", "USD", None), ("USD", "USD", None, None)], commodities
        )
    ...
    def test_aggregate_holdings_list(self):
        ...
        expected_holding = holdings.Holding(
            ...
            D("15572.10"),
            D("17402.20"),
            D("580.0733333333333333333333333"),
            None,
        )
    ...
    def test_aggregate_holdings_by__commodity(self):
        ...
        test_holdings = list(
            itertools.starmap(
                holdings.Holding,
                [
                    ("Assets:Cash", D("101.11"), "USD", None, None, None, None, None, None),
                    (
                        "Assets:Account1",
                        D("10"),
                        "HOOL",
        ...
        self.assertEqual(
            expected_holdings,
            holdings.aggregate_holdings_by(test_holdings, lambda holding: holding.currency),
        )
    ...
    def test_aggregate_holdings_by__account(self):
        ...
        self.assertEqual(
            expected_holdings,
            holdings.aggregate_holdings_by(test_holdings, lambda holding: holding.account),
        )
    ...
    def test_convert_to_currency(self, entries, _, __):
        ...
        none_holding = holdings.Holding(
            None, D("100.00"), None, D("200"), None, None, None, None, None
        )
        with self.assertRaises(ValueError):
            converted_holdings = holdings.convert_to_currency(
                price_map, "USD", [none_holding]
            )
