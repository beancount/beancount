...
class Params:
    ...
    x_txn_offset = int(post_radius * 2 * 1.2)                             # Offset of txn node to the right of the posting nodes
    x_section_margin = x_txn_offset + txn_radius + x_margin    # Horizontal margin between sections
    ...
    assert (x_timeline_before +
            x_timeline_open +
            x_timeline_period +
            x_timeline_close +
            x_timeline_after) < x_timeline_length
...
def main():
    ...
    parser.add_argument('-b', '--balances-filename', action='store',
    ...
    with open(args.balances_filename, 'r') as balfile:
        balfile.readline()
        balances = sorted([(account, balance)
                           for (account, balance) in csv.reader(balfile)
                           if (not re.search(r'201\d', account) and
                               re.search(r'\bUSD\b', balance) and
                               balance.strip())])
...
def draw_line(p, y, acctype, account_name, balance_amount, pr):
    ...
    pr('')
    pr('<line x1="{x}" y1="{y}"  x2="{x2}" y2="{y}" stroke="#000" marker-end="url(#arrow)" />'.format(
        x=p.x_timeline_start, x2=p.x_timeline_end, y=y))

    if p.draw_acctype:
        pr('<text x="{x}" y="{y}" text-anchor="end" alignment-baseline="central" font-family="Helvetica" font-weight="bold">'.format(
            x=p.x_margin + p.x_acctype_width, y=y))
        pr(acctype)
        pr('</text>')

    if p.draw_account:
        pr('<text x="{x}" y="{y}" text-anchor="start" alignment-baseline="central" font-family="Helvetica" font-weight="bold">'.format(
            x=p.x_margin + p.x_acctype_width, y=y))
        pr(account_name)
        pr('</text>')

    if p.draw_balance:
        pr('<text x="{x}" y="{y}" text-anchor="end" alignment-baseline="central" font-family="Courier">'.format(
...
def draw_txn(p, x, y_selected, y_lines, pr, debug=False):
    ...
    for y in y_selected:
      pr('<path d="M {} {} C {} {}, {} {}, {} {}" fill="transparent" />'.format(
          x, y, xt, y, xt, (yt + y)/2, xt, yt))

    # Draw the post icons (circles).
    for y in y_selected:
      pr('<circle cx="{x}" cy="{y}" r="{r}" fill="#FFF" />'.format(
