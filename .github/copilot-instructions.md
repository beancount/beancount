...
- `crates/beancount-parser/`: the parser that parse string into tree-sitter Node and convert them to our internal AST and core rust type `CoreDirective`.
...
2. the crate `crates/beancount-parser-py/` will compiled to `beancount/parser/_parser_rust`, when you update the signature, you must update type-stub accordingly.
